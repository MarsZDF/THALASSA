function [x0new, Delx_Vec, PDelx_Mat, RRi] = NLSCamera(Xref, Xobs, T, Qi, RTensor)
...
for tt = 1:length(T)
    ...
    QiInv(:,:,tt) = inv(Qi(1:3,1:3,tt)); 
    RRi(tt,:) = Xobs(tt,1:3)' - (RTensor(:,:,tt)*Xref(tt,1:3)'); %Obtain the residuals
    Hi(1:3,1:3, tt) = RTensor(:,:,tt);
    Ti(:,:,tt) = Hi(:,:, tt)*PhiT(:,:,tt);
    if tt > 1
